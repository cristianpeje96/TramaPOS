...
async def crear_devolucion(datos: DevolucionCrear, db: AsyncSession = Depends(get_db)):
    try:
...
async def devolucion_de_venta(venta_id: int, db: AsyncSession = Depends(get_db)):
