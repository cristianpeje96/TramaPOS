...
import { ArrowLeft, Package, AlertTriangle, Receipt, Gift, Award, Percent, Store, Truck, ShoppingBag, BarChart2, Sparkles, Wallet, FileText } from 'lucide-react';
...
import CotizacionesAdmin from '../CotizacionesAdmin/CotizacionesAdmin';
import { useAuth } from '../../Auth/AuthProvider';
import './AdminScreen.scss';

const PESTANAS = [
  { id: 'reportes', label: 'Reportes', icon: BarChart2, Componente: ReportesAdmin },
  { id: 'finanzas', label: 'Finanzas', icon: Wallet, Componente: FinanzasAdmin },
  { id: 'cotizaciones', label: 'Cotizaciones', icon: FileText, Componente: CotizacionesAdmin },
  { id: 'asistente', label: 'Asistente IA', icon: Sparkles, Componente: AsistenteIA },
  { id: 'productos', label: 'Productos', icon: Package, Componente: ProductosAdmin },
  { id: 'stock-bajo', label: 'Stock bajo', icon: AlertTriangle, Componente: StockBajoPanel },
  { id: 'ventas', label: 'Historial ventas', icon: Receipt, Componente: HistorialVentas },
  { id: 'puntos', label: 'Historial puntos', icon: Gift, Componente: HistorialPuntos },
  { id: 'fidelizacion', label: 'Fidelización', icon: Award, Componente: ConfiguracionFidelizacion },
  { id: 'iva', label: 'IVA', icon: Percent, Componente: ConfiguracionIVA },
  { id: 'cajas', label: 'Cajas físicas', icon: Store, Componente: CajasFisicasAdmin },
  { id: 'proveedores', label: 'Proveedores', icon: Truck, Componente: ProveedoresAdmin },
  { id: 'compras', label: 'Compras', icon: ShoppingBag, Componente: ComprasAdmin },
];
  ...
  return (
    <div className="admin-screen">
      <header className="admin-screen__topbar">
        <button type="button" className="admin-screen__volver" onClick={onVolverAlPos}>
        ...
        <nav className="admin-screen__tabs">
          {PESTANAS.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              type="button"
              className={`admin-screen__tab ${pestanaActiva === id ? 'admin-screen__tab--activa' : ''
                }`}
